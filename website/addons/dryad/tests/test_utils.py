...
class DryadTestUtils(DryadTestCase):
    ...
    def setUp(self):
        ...
        self.repository = DryadRepository(self.connection)

    def test_get_dryad_title(self):
        resp = self.repository.get_dryad_title(doi="doi:10.5061/dryad.1850")
        assert_true("phylogeny" in resp)

    def test_check_dryad_doi(self):
        resp = self.repository.check_dryad_doi(doi="doi:10.5061/dryad.1850")
        assert_true(resp)
